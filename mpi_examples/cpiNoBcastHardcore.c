...
//	MPI_Recv(&n, 1, MPI_INT, MPI_ANY_SOURCE, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
  //  }
    ...
    for (y = 0; y <= log(numprocs) + 1; y++) {
        int procs = pow(2, y);
	if (myid < procs) {
	    MPI_Send(&n, 1, MPI_INT, myid + procs, 0, MPI_COMM_WORLD);
	} else if (myid < procs * 2) {
	    MPI_Recv(&n, 1, MPI_INT, myid - procs, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
	}
