...
/*
    if (myid == 0) {
        int sendTo = 1;
        for (sendTo = 1; sendTo < numprocs; sendTo++) {
            MPI_Send(&n, 1, MPI_INT, sendTo, myid, MPI_COMM_WORLD);
        }
    }
    if (myid != 0) {
...
*/
    ...
    int base = round(pow(2.0, (double)myid));
    while (1) {
        int recipient = base + myid;
        if (recipient >= numprocs) break;
        MPI_Send(&n, 1, MPI_INT, recipient, 0, MPI_COMM_WORLD);
        base *= 2;
    }
